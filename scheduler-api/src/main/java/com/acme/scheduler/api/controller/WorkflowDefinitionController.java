...
public class WorkflowDefinitionController {
	...
	@Operation(summary = "Upsert a workflow definition", description = "Creates a new workflow definition or updates an existing one. The request carries task definitions and DAG edges as JSON.\n\nTenant routing: provide tenant id via the X-Tenant-Id request header.\n\nNotes: this endpoint only persists the definition; execution is started separately via the execution APIs.")
	@ApiResponses({
			@ApiResponse(responseCode = "200", description = "Workflow definition persisted (workflowCode + workflowVersion returned).", content = @Content(schema = @Schema(implementation = CreateWorkflowDefinitionResponse.class))),
	...
	public ResponseEntity<?> create(@RequestHeader("X-Tenant-Id") String tenantId,
			@RequestBody CreateWorkflowDefinitionRequest req) throws Exception {
		...
		var r = useCase.handle(tenantId, req.getName(), req.getSlaSeconds(), tasks, edges, req.getWorkflowCode());
	...
	public static class CreateWorkflowDefinitionRequest {
		...
		public void setSlaSeconds(Long slaSeconds) {
