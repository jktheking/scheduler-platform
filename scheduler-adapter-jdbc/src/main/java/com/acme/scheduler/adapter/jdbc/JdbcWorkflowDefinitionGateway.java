...
public final class JdbcWorkflowDefinitionGateway implements WorkflowDefinitionGateway {
	...
	@Override
	public UpsertResult upsertWorkflowDefinition(String tenantId, Long workflowCodeOrNull, String name,
			List<TaskSpec> tasks, List<EdgeSpec> edges, Long slaSecondsOrNull) {
		...
		String definitionJson = buildDefinitionJson(tasks, edges, slaSecondsOrNull);
		...
		jdbc.update("DELETE FROM t_workflow_dag_edge WHERE workflow_code=? AND workflow_version=?", workflowCode,
				workflowVersion);
	...
	/**
	 * Minimal JSON builder to avoid forcing core to depend on Jackson. Adapter
	 * layer is allowed to build strings.
	 */
	private static String buildDefinitionJson(List<TaskSpec> tasks, List<EdgeSpec> edges, Long slaSecondsOrNull) {

		StringBuilder sb = new StringBuilder();
		sb.append('{');
		if (slaSecondsOrNull != null && slaSecondsOrNull.longValue() > 0L) {
			sb.append("\"slaSeconds\":").append(slaSecondsOrNull.longValue()).append(',');
		}
		sb.append("\"tasks\":[");
		for (int i = 0; i < tasks.size(); i++) {
			...
			if (i > 0)
				sb.append(',');
			sb.append('{').append("\"taskCode\":").append(t.taskCode()).append(',').append("\"taskVersion\":")
			.append(t.taskVersion()).append(',').append("\"name\":\"").append(jsonEscape(t.name()))
			.append("\",").append("\"taskType\":\"").append(jsonEscape(t.taskType())).append("\",")
		...
		for (int i = 0; i < edges.size(); i++) {
			...
			if (i > 0)
				sb.append(',');
			sb.append('{').append("\"fromTaskCode\":").append(e.fromTaskCode()).append(',').append("\"toTaskCode\":")
			.append(e.toTaskCode()).append('}');
		}
		sb.append(']');
		sb.append('}');
		return sb.toString();
