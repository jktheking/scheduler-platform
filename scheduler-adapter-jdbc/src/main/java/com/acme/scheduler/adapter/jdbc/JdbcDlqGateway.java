...
public final class JdbcDlqGateway implements DlqGateway {
	...
	public void replay(long dlqId) {
		var rows = jdbc.queryForList("SELECT workflow_instance_id, task_instance_id FROM t_dlq_task WHERE dlq_id=?",
				dlqId);
		if (rows.isEmpty())
			return;
		...
		jdbc.update("""
				...
				""", workflowInstanceId, taskCode, taskVersion, taskName, taskType, newAttempt, newMaxAttempts,
		...
		// Schedule a wakeup trigger so master publishes the retry attempt to ready
		// topic.
		jdbc.update("""
